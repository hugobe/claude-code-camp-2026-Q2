...
We want to use the standard library as much as possible avoiding external packages. We add `PyYAML` for settings and `python-dotenv` to be able to load `.env` files.
...
| `boukensha/tasks/base.py` | abstract `boukensha.tasks.Base` (provider/model + prompt resolution) |
| `boukensha/tasks/player.py` | concrete `boukensha.tasks.Player` (the main loop) |
