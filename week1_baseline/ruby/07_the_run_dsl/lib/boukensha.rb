...
module Boukensha
  ...
  #   backend:      :anthropic (default), :openai, :gemini, :ollama, :ollama_cloud, or :lm_studio.
  ...
  #                 env var (loaded from .boukensha/.env). Not needed for :ollama or :lm_studio.
  #   ollama_host:  Ollama base URL. Defaults to "http://localhost:11434".
  #   lm_studio_host: LM Studio base URL. Defaults to "http://localhost:1234/v1".
  ...
  def self.run(
    ...
    be = case backend
         ...
         when :lm_studio    then Backends::LmStudio.new(host: lm_studio_host, model: model)
         else raise ArgumentError, "Unknown backend #{backend.inspect}. Use :anthropic, :openai, :gemini, :ollama, :ollama_cloud, or :lm_studio."
         end
...
require_relative "boukensha/backends/lm_studio"
