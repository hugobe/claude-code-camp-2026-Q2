---
name: play-mud
description: Play tbaMUD/CircleMUD games over telnet by driving a persistent socket connection turn by turn, and pursue longer-horizon goals (like "reach level 7" or "hunt down and kill the swamp troll") that span multiple sessions by reading and updating data/player.db and data/world.db (via scripts/memory.py) as persistent, queryable memory. Use this whenever the user wants to explore, play, test, or interact with a MUD (Multi-User Dungeon) running locally, especially the one at localhost:4000 with the dummy:helloworld character — including navigating rooms, fighting, managing inventory, talking to NPCs, tracking progress toward a goal, or just checking what's happening in the game world right now. Push to use this skill any time "MUD", "tbaMUD", "CircleMUD", or "localhost:4000" comes up, even if the user just says something like "log in and see what's around," "attack that rat," or "keep working on my mud goal" without spelling out the full task.
---
...
Default target (already configured, no extra input needed unless the user
says otherwise): `localhost:4000`, character `dummy` / password `helloworld`.
...
`data/player.db` stores a small persona alongside it: `playstyle` (free
text — a short description like "aggressive melee" or "cautious
...
## Long-term memory: data/player.db and data/world.db
...
Persistent memory lives in two small SQLite databases, managed entirely
through `scripts/memory.py` — never edit the `.db` files by hand:

- **`data/player.db`** — this character's current goal, live stats
...
- **`data/world.db`** — facts about the game world itself: rooms and the
  ...
  and unresolved leads. This is deliberately a *separate* file from
  player.db, not just a separate set of tables, because it's a different
  kind of fact with a different lifetime — the map and shop prices don't
  change depending on who's playing, so this file could in principle be
  shared across multiple characters someday, while player.db is tied to
  this one character's progress.
...
multiple independent sessions or agents touching the same character.
...
- **Present the plan — as a heads-up, not a gate.** Once the queue
  ...
  the full `dump` output. The queue living in `data/player.db` makes it
  durable; it doesn't make it *communicated* — a plan the user never
  sees isn't one they can correct or react to. This is purely
...
what to type next. The task queue in `data/player.db` bridges that gap:
as soon as a goal is set (or changed), break it down into a short
...
```
python3 scripts/memory.py task add "Grind exp at Watery Sewer Bend (sewer rats/spiders) - ~2,400 exp to level 4" --status active --priority 1
python3 scripts/memory.py task add "Sell surplus loot at the Weapon Shop once inventory fills up" --priority 2
python3 scripts/memory.py task add "Try the Guild of Magic Users bar" --status blocked --reason "guarded, revisit after leveling"
```
...
else; `data/player.db` is the only place all of them can actually see
current status. That's why `priority` is a plain number rather than a
...
Every subcommand also takes `--help` for the full option list.

## Workflow

1. **Start the session** (once, at the beginning of play):
   ```
   python3 scripts/memory.py dump
...
5. **End the session** when done playing:
   ...
   `data/player.db` and `data/world.db` via `scripts/memory.py` (see
   above) with the latest state and progress, so the next session can
...
- If `scripts/mud.py start` errors out immediately, check
  `/tmp/mud-skill-4000/daemon.stderr.log` for the underlying connection
  error (e.g. the MUD server isn't actually running on that port).
- `memory.py` commands hang or raise `database is locked` — another
  ...
  brief overlap automatically, so a hang past that means something is
  stuck, not just contended — retry once. `data/*.db` don't exist yet on
  first run — every `memory.py` command auto-creates/upgrades the schema,
  so just run any subcommand (`dump` is a good first one).
