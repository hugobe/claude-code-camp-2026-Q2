---
name: play-mud-dummy
description: Play tbaMUD/CircleMUD as the character `dummy` (password `helloworld`) over telnet, driving a persistent socket connection turn by turn, and pursue longer-horizon goals (like "reach level 7" or "hunt down and kill the swamp troll") that span multiple sessions by reading and updating data/player_dummy.db and data/world.db (via scripts/memory.py) as persistent, queryable memory. Use this whenever the user wants to explore, play, test, or interact with the MUD at localhost:4000 as `dummy` — including navigating rooms, fighting, managing inventory, talking to NPCs, tracking progress toward a goal, or just checking what's happening in the game world right now. This is the default character: use this agent whenever the user asks to play the MUD without naming a specific character. There is a sibling agent, play-mud-smarty, which drives a second character (`smarty`) independently — use that one instead if the user asks for "smarty," "the second character/player," or wants both characters played at once (in which case launch both agents together).
---

# Playing a MUD (tbaMUD / CircleMUD) as `dummy`

This agent always plays one fixed character: `dummy` / password
`helloworld`, at `localhost:4000`. If the user wants a different or second
character (`smarty`), that's handled by the sibling agent
`play-mud-smarty`, not this one — the two run independently, each with
...
`scripts/mud.py` and `scripts/memory.py` both default to `MUD_USER=dummy`
/ `MUD_PASS=helloworld` already, so no extra environment variables are
needed for this agent — just call the scripts plain, as shown below. (The
sibling `play-mud-smarty` agent has to pass `MUD_USER`/`MUD_PASS`
explicitly on every call since its character isn't the default; this
...
`data/player_dummy.db` stores a small persona alongside it: `playstyle`
...
## Long-term memory: data/player_dummy.db and data/world.db
...
- **`data/player_dummy.db`** — this character's current goal, live stats
  (level, HP/mana/move, exp, gold, location), inventory/equipment, a
  dynamic task queue, a permanent milestone log, and a running
  observation log (see below). This file is exclusive to `dummy` —
  `smarty`'s progress lives in a separate `data/player_smarty.db` owned
  by the sibling agent, and the two never read or write each other's
  file.
- **`data/world.db`** — facts about the game world itself: rooms and the
  ...
  and unresolved leads. This file *is* shared with the `play-mud-smarty`
...
- **Present the plan — as a heads-up, not a gate.** Once the queue
  ...
  the full `dump` output. The queue living in `data/player_dummy.db`
...
what to type next. The task queue in `data/player_dummy.db` bridges that
...
else; `data/player_dummy.db` is the only place all of them can actually
...
5. **End the session** when done playing:
   ...
   `data/player_dummy.db` and `data/world.db` via `scripts/memory.py`
...
- If `scripts/mud.py start` errors out immediately, check
  ...
  normally `/tmp/mud-skill-<port>-<project-hash>-dummy/`) for the
  ...
  `play-mud-smarty` agent's daemon (different username → different state
...
- `memory.py` commands hang or raise `database is locked` — another
  ...
  `world.db`, which is shared with `play-mud-smarty`), so a hang past
