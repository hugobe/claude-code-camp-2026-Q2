...
databases:
  ...
  data/player_<character>.db  - that character's goal, live stats, task
                     queue, inventory, milestones, and a running
                     observation log. <character> comes from the MUD_USER
                     env var (same one scripts/mud.py uses to log in),
                     defaulting to "dummy" - so each character (e.g.
                     "dummy" or "smarty") gets its own independent memory
                     without any extra flags to remember.
  data/world.db   - facts about the game world itself: rooms, exits,
                     shops, NPCs/monsters, unresolved leads. Shared across
                     all characters.

Why two databases instead of one, and why SQLite instead of markdown:

- Two files (well, one-per-character plus a shared one) because "what
  this character is trying to do" and "what the world looks like" are
  different kinds of fact with different owners and different lifetimes -
  world.db is shared across every character/session playing the same MUD,
  while each player_<character>.db is tied to one character's progress.
...
CHARACTER = os.environ.get("MUD_USER", "dummy")
PLAYER_DB = os.path.join(DATA_DIR, f"player_{CHARACTER}.db")
...
def cmd_init(args):
    init_dbs()
    print(f"Initialized {PLAYER_DB} and {WORLD_DB} (character: {CHARACTER})")
...
def cmd_dump(args):
    ...
    print("=" * 60)
    print(f"PLAYER MEMORY (character: {CHARACTER})")
    print("=" * 60)
    print(f"\nGoal: {_goal_line(pconn)}")
