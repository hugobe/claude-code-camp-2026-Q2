...
def _start_daemon(wait_login: float = 6.0):
    """Spawn the background daemon if not already running. Returns (started, output).
    `started` is False if it was already up or failed to come up; `output` is any
    text produced so far (login/MOTD), or an error message on failure."""
    if _daemon_running():
        return False, _read_new_output()
    os.makedirs(STATE_DIR, exist_ok=True)
...
# --- HTTP bridge (for n8n AI Agent Code Tool / HTTP Request Tool) ----------
#
# Pyodide (n8n's Python Code node/tool) has no raw sockets, so the MUD telnet
# connection can't live inside n8n. Instead this process holds the persistent
# daemon and exposes a tiny JSON-over-HTTP control surface that an n8n JS Code
# Tool (or the native HTTP Request Tool) can call:
#
#   POST /send    {"text": "look", "wait": 1.5}  -> {"ok": true, "output": "..."}
#   GET  /read                                     -> {"ok": true, "output": "..."}
#   GET  /status                                   -> {"ok": true, "running": .., "connected": ..}
#   POST /start                                    -> {"ok": true, "output": "..."}   (auto-login)
#   POST /stop                                     -> {"ok": true}
#
# Optional shared-secret: set MUD_HTTP_TOKEN and send `Authorization: Bearer <token>`.


def cmd_serve_http(args):
    from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

    token = os.environ.get("MUD_HTTP_TOKEN")
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if self.path.startswith("/read"):
                return self._reply(200, {"ok": True, "output": _read_new_output()})
            if self.path.startswith("/status"):
                running = _daemon_running()
                connected = None
                if running:
                    try:
                        connected = _control_request({"cmd": "ping"}).get("connected")
                    except OSError:
                        pass
                return self._reply(200, {"ok": True, "running": running, "connected": connected})
        ...
        def do_POST(self):
            ...
            if self.path.startswith("/send"):
                if not _daemon_running():
                    return self._reply(409, {"ok": False, "error": "daemon not running; POST /start first"})
                resp = _control_request({"cmd": "send", "text": req.get("text", "")})
                if not resp.get("ok"):
                    return self._reply(502, {"ok": False, "error": resp.get("error")})
                try:
                    wait = float(req.get("wait", 1.0))
            ...
            if self.path.startswith("/start"):
                started, out = _start_daemon()
                return self._reply(200, {"ok": True, "started": started, "output": out})
    ...
    print(f"MUD HTTP bridge on http://{args.host}:{args.port}  (token {'set' if token else 'disabled'})")
    try:
...
def main():
    ...
    p_http = sub.add_parser("serve-http", help="Run a JSON/HTTP bridge to the daemon (for n8n).")
    p_http.add_argument("--host", default=os.environ.get("MUD_HTTP_HOST", "127.0.0.1"))
    p_http.add_argument("--port", type=int, default=int(os.environ.get("MUD_HTTP_PORT", "8765")))
